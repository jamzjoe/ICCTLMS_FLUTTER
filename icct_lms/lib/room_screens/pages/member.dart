...
class _MemberState extends State<Member> {
  ...
  Stream<List<MembersModel>> getMembers() => FirebaseFirestore.instance
      .collection('Rooms')
      .doc(widget.roomType)
      ...
      .snapshots()
      .map((event) =>
          event.docs.map((e) => MembersModel.fromJson(e.data())).toList());
  ...
  Widget buildMemberTiles(MembersModel e) => Card(
      ...
      child: ListTile(
          leading: CircleAvatar(
            backgroundColor:
                e.userType == 'Teacher' ? Colors.white : Colors.blue[900],
            child: Center(
              child: Text(
          ...
          trailing: widget.userType == 'Student' && widget.uid != e.userID
              ...
              : widget.userType == "Student"
                  ...
                  : widget.userType == 'Teacher' && widget.uid != e.userID
                      ? PopupMenuButton(
                          itemBuilder: (context) => [
                                PopupMenuItem(
                                    onTap: () async {
  ...
  Future showNotice(String name, String userID, String userType) => showDialog(
      ...
      builder: (context) => AlertDialog(
            ...
            content: widget.userType == 'Teacher'
                ? Text('Are you sure you want to '
                    'remove '
                    '$name in this ${widget.roomType}?')
                : Text(
                    'Are you sure you want to leave in this ${widget.roomType}?'),
            actions: [
              TextButton(
                  onPressed: () async {
                    ...
                    await join.deleteJoin(widget.roomType, widget.roomCode,
                        userID, widget.teacherUID);
                  },
  ...
  Future showWarning() => showDialog(
      ...
      builder: (context) => AlertDialog(
            title: const Text("Warning"),
            content: Text(
                'Are you sure you want to remove this ${widget.roomType}?'),
            actions: [
              TextButton(
                  onPressed: () async {
                    ...
                    await service.deleteRoom(
                        widget.roomType, widget.uid, widget.roomCode);
                    await post.deleteEachRoomPost(widget.roomCode);
                  },
                  child: const Text('Okay')),
              TextButton(
                  onPressed: () {
